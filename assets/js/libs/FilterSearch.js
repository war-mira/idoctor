class FilterSearch {
  ...
  appendItemToList (str) {
    const body = this.instance.closest('.search--filters__item-body')
    const default_list = body.querySelector('.search--filters__container')
    this.input.value = ''
    const item = document.createElement('div')
    item.className = 'param_name tag tag--light tag--medium'
    item.innerHTML = `${str}`
    default_list.append(item)
  }
}

document.querySelectorAll('.filter_mainsearch').forEach(item => {
  const search = new FilterSearch(item)
  search.init()
})
