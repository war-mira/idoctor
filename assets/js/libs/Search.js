...
class Search {
  constructor (instance) {
    ...
    this.cityName = this.instance.querySelector('.search--alias').value
    this.countryAlias = this.instance.querySelector('.search--country').getAttribute('data-country')
  }
  ...
  listenInputFocusing () {
    this.input.addEventListener('focus', (e) => {
      ...
      if (this.form.getAttribute('data-source')){
  ...
  createSearchOption (item) {
    ...
    let alias = this.form.querySelector('.search--alias').getAttribute('data-city')
    let img = ''
    let link = ''
    if ((item.type == 'medcenter') || (item.type == 'doctor')) {
      ...
      link = `/search?query=${encodeURI(item.name)}`
    }

    let template =
      `<a href=${link}><div class="icon-btn--wrapper icon-btn--gray system--search__line">
          ${img}
...
document.querySelectorAll('.system--search').forEach(item => {
  let wrapper = new Search(item)
  wrapper.init()
})
