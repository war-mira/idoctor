...
class SelectCity {
  constructor (instance) {
    ...
    this.search_attr = instance.getAttribute('search_attr')
      ? JSON.parse(instance.getAttribute('search_attr'))
      : ''
      ...
      socket.on('search results', msg => {
        if (this.iid === msg.key) {
          ...
          this.instance.dispatchEvent(
            new CustomEvent('autocomplete_selected', { detail: value })
          )
        }
      })
    }, 300)
  ...
  initInput () {
    this.input.addEventListener('input', e => {
      ...
      socket.emit(
        'search inserting',
        val,
        this.search_attr.length ? this.search_attr : ['name'],
        this.model,
        this.iid
      )
    })
  ...
  optionsChanged () {
    ...
    if (this.options.length && this.input.value.length) {
      this.optionsInstance.querySelector(
        '.select--city__list-default'
      ).hidden = true
      ...
      while (i > 0) {
        ...
        if (typeof item !== 'undefined') {
          item.addEventListener('click', e => {
            ...
            socket.emit(
              ...
              this.options.filter(item => {
                return (
                  item.value.toLocaleLowerCase() == content.toLocaleLowerCase()
                )
              }),
              this.iid
            )
          ...
          this.optionsInstance
            .querySelector('.select--city__list')
            .appendChild(item)
        }
        i--
      }
